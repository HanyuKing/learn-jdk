...
public class ImageUtil {
    /**
     * 从图片 URL 下载图像，按指定区域裁剪，并保存到本地文件
     * @param imageUrl 图片的 URL
     * @param outputFilePath 输出文件的本地路径（如 D:/output.jpg）
     * @param x 裁剪起始 X 坐标
     * @param y 裁剪起始 Y 坐标
     * @param width 裁剪宽度
     * @param height 裁剪高度
     * @throws Exception 抛出任意异常，调用者自行处理
     */
    public static void cutImageFromUrl(String imageUrl, String outputFilePath) throws Exception {
        ...
        try (InputStream inputStream = url.openStream()) {
            ...
            // 裁剪图像
            BufferedImage croppedImage = originalImage.getSubimage(x, y, width / 2, y);

            // 确定输出文件格式（如 jpg, png）
            String formatName = getFormatName(outputFilePath);

            // 输出文件
            File outputFile = new File(outputFilePath);
            ImageIO.write(croppedImage, formatName, outputFile);
            System.out.println("图片保存路径：" + outputFile.getAbsolutePath());
        }
    }
     ...
     * @throws Exception 抛出任意异常，调用者自行处理
     */
    public static void cutImageFromUrl(String imageUrl, String outputFilePath, int x, int y, int width, int height) throws Exception {
        // 读取网络图片为 BufferedImage
        URL url = new URL(imageUrl);
        try (InputStream inputStream = url.openStream()) {
            BufferedImage originalImage = ImageIO.read(inputStream);
...
//             Integer MOVE = number.divide(olDnumber).intValue()*150;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //添加文字水印
            ImageUtil.textWatermarking_Image(inputFilePath, outFilePath, "版权所有", -45, 80, 80);
    ...
    private static String getFormatName(String filePath) {
        ...
        if (dotIndex == -1 || dotIndex == filePath.length() - 1) {
            throw new IllegalArgumentException("输出文件路径必须包含有效的文件扩展名，如 .jpg 或 .png");
        }
